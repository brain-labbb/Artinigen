...
|---|---|---|---|---|
| `pedestal_base` | S1 | `model.py:L148-L173` | eligible if compatible | cylindrical base/pedestal with bearing rings |
| `roof_mount` | S2 | `model.py:L146-L193` | eligible if compatible | roof plate / low mount, compact installation |
| `tripod_base` | S3 | `model.py:L115-L158` | eligible if compatible | three legs and spreaders, portable support |
| `mast_mount` | S5 | `model.py:L129-L148` | eligible if compatible | vertical mast carrying azimuth fork |
| `heavy_service_base` | S6 | `model.py:L93-L130` | eligible if compatible | slab base, yoke towers, rear cable box |

### Slot B：azimuth_stage
| module_name | 5_star_source | model.py:Lx-Ly | sampling eligibility | 结构特征 |
|---|---|---|---|---|
| `dual_yoke_stage` | S1 | `model.py:L185-L228,L320-L326` | eligible if compatible | left/right yoke cheeks on azimuth turntable |
| `compact_roof_fork` | S2 | `model.py:L205-L251,L336-L342` | eligible if compatible | short azimuth fork and bearing ring |
| `tripod_head` | S3 | `model.py:L158-L171,L254-L260` | eligible if compatible | compact portable head, REVOLUTE yaw |
| `mast_azimuth_fork` | S5 | `model.py:L170-L183,L327-L333` | eligible if compatible | mast-top fork with annular collar |
| `instrument_yoke` | S4 | `model.py:L232-L279,L410-L416` | eligible if compatible | yoke arms, rear braces, instrument support |

### Slot C：dish_reflector_assembly
| module_name | 5_star_source | model.py:Lx-Ly | sampling eligibility | 结构特征 |
|---|---|---|---|---|
| `lathe_reflector_with_feed` | S1 | `model.py:L240-L308,L329-L335` | eligible if compatible | lathe reflector, rim, feed boom/horn, rear supports |
| `roof_feed_boom_dish` | S2 | `model.py:L64-L104,L266-L324` | eligible if compatible | mesh reflector, feed boom, ribs, support struts |
| `portable_ribbed_dish` | S3 | `model.py:L55-L91,L191-L247` | eligible if compatible | dish shell, ribs, feed arm for tripod |
| `instrumented_dish_with_box` | S4 | `model.py:L291-L398,L419-L446` | eligible if compatible | dish assembly plus fixed instrument box and hinged hatch |
| `cadquery_spoked_reflector` | S5 | `model.py:L27-L104,L209-L320` | eligible if compatible | cadquery dish shell, parabolic spokes, fork-side axis |

### Slot D：auxiliary_mechanism
| module_name | 5_star_source | model.py:Lx-Ly | sampling eligibility | 结构特征 |
|---|---|---|---|---|
| `none` | S1 | `model.py:L320-L335` | eligible if compatible | only azimuth + elevation |
| `instrument_hatch` | S4 | `model.py:L368-L446` | eligible if compatible | fixed instrument_box plus REVOLUTE hatch |
| `rear_cover` | S6 | `model.py:L171-L201` | eligible if compatible | dish-mounted rear cover hinged on electronics frame |
| `side_crank_or_lock_knob` | S4 | `model.py:L232-L279` | eligible if compatible | visual/mechanical side crank or lock knob on azimuth/elevation head |
...
| `azimuth_joint_type` | enum | `continuous`, `revolute_limited` | `continuous` | samples include both; default continuous | S1 / S3 |
...
预计 `module_topology_diversity` 门控（>=10 distinct）能否过：yes。理由：root support, auxiliary hatch/rear cover, azimuth joint type, reflector construction and fixed instrument box add distinct topologies.
