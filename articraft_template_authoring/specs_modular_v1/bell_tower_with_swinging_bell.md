...
| bell + clapper / striker secondary motion | 15 | clapper 或 external striker 作为第二 REVOLUTE；若 bell 固定而只有 clapper 动，则不作为 seed=0 anchor |
...
|---|---|---|---|---|
| `steel_x_braced_frame` | S1 | `model.py:L133-L348` | **yes** | 四柱 frame、baseplate、top beam、X bracing、pivot plates，开放式工业 bell tower |
| `octagonal_rooftop_cupola` | S2 | `model.py:L201-L316` | | roof_plate + octagonal cupola + louver panels，cupola FIXED 到 roof |
| `raked_timber_cote` | S3 | `model.py:L102-L165` | | 双 raked upright + gabled apex + arched yoke support，窄 timber bell cote |
| `gothic_masonry_belfry` | S4 | `model.py:L223-L441` | | stone shaft、pointed arch / octagonal spire、可容纳多 bell 的 belfry tier |
| `japanese_post_beam_pavilion` | S6 | `model.py:L66-L168` | | 重 timber post-and-beam frame，适合 bonsho bell + striker |

### Slot B：bell_and_yoke
| module_name | 5_star_source | model.py:Lx-Ly | seed=0 anchor | 结构特征 |
|---|---|---|---|---|
| `bronze_bell_with_steel_yoke` | S1 | `model.py:L86-L118,L348-L399` | **yes** | lathe bell shell、front/back yoke arm、boss、crossbar，bell_assembly 整体 swing |
| `arched_hanger_bell` | S3 | `model.py:L31-L98,L165-L256` | | bell shell + arched yoke + hanger cheeks，bell 与 clapper 分别为 part |
| `cupola_cast_bell` | S2 | `model.py:L166-L192,L328-L383` | | cupola 内 bell + fixed clapper visual/part，适合小 rooftop cupola |
| `bonsho_suspended_bell` | S6 | `model.py:L34-L58,L175-L211` | | 直壁 bonsho-style shell，无西式大 yoke，配 external striker |

### Slot C：secondary_motion
| module_name | 5_star_source | model.py:Lx-Ly | seed=0 anchor | 结构特征 |
|---|---|---|---|---|
| `none` | S1 | `model.py:L399-L405` | **yes** | 只有主 bell REVOLUTE，最稳健 |
| `internal_clapper_revolute` | S3 | `model.py:L213-L276` | | clapper part 挂在 bell 下，绕同水平轴小角度摆动 |
| `external_striker_revolute` | S6 | `model.py:L211-L262` | | striker 挂 frame，绕反向水平轴撞 bell |
| `guide_wheel_continuous` | S5 | `model.py:L309-L331` | | guide_wheel / pulley 在 tower 上 CONTINUOUS，辅助 rope guide |
...
| `bell_count` | int | `1..3` | `1` | multi-bell 只在 masonry/open frame 子域采样；seed=0 为 1 | S4 |
...
预计 `module_topology_diversity` 门控（>=5 distinct）能否过：yes。理由：support part trees、secondary joint count、multi-bell count、cupola fixed stack、bonsho striker topology 都产生 distinct part/joint skeletons。
