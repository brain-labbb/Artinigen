...
3. **Multiplicity**（monitor_mount arms / fan blades）：一个槽位有
   ...
   "同一种 visual/part/joint 或 part+joint 组合重复 N 次"。spec 必须写清
   可采样范围、离散集合或固定 N 个同构复制件，例如 fan blades `3..8`、
   serial arm links `1..5`、three branches `fixed 3`；不要只写抽象的 `N`。
   固定单件 named slot（one propeller / one door / one car）不算 Multiplicity。
...
- 参数范围表
- Multiplicity / Copy Logic（每个 spec 必填；agent 自己判断；有同构复制件
  才写具体复制规则和 `N_min..N_max` / 离散集合 / 固定 N 个同构复制件；
  没有就写 "无复制数量逻辑"，不要硬凑 `*_count=fixed 1`）
