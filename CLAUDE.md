...
If the user asks Claude Code to author or improve a procedural template under `agent/templates/`, follow [`TEMPLATE_AUTHORING_AGENT.md`](TEMPLATE_AUTHORING_AGENT.md). The authoritative per-iteration signal is `uv run articraft template sweep-pipeline <slug>`; do not declare a template done based on pytest, QC scripts, or visual inspection alone. The spec-stage workflow in `articraft_template_authoring/agent_workflow.md` §1–3 is unchanged; only the post-review iteration loop (§4.6) is superseded.
